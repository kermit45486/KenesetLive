...
Opinions (Deot) are fetched from the external Deot Web Service via HTTP.
"""
from functools import wraps
import os
import uuid
import requests as http_requests  # type: ignore  # HTTP client for calling the Web Service
...
# ── Deot Web Service URL ───────────────────────────────────────────
DEOT_API_URL = "http://localhost:5000/api/deot"
...
def add_dea():
    ...
    try:
        ...
        if resp.status_code == 201:
            flash("הדעה נוספה בהצלחה!", "success")
        else:
